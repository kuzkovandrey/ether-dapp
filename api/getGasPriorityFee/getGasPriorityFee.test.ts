...
describe("[Mock api call - positive cases]: _getGasPriorityFee function", () => {
  ...
  beforeEach(() => {
    ...
    } = generateRewards(blocks);

    rewards = generatedRewards;
    expectedPriorityFees = generatedExpectedPriorityFees;

    mockProvider = {
      send: (...args: any[]): Promise<FeeHistory> =>
        Promise.resolve({
          reward: rewards,
        }),
      getBlock: (...args: any[]): Promise<Block> =>
        Promise.resolve({ baseFeePerGas: parseUnits("1", "gwei") } as Block),
    } as JsonRpcProvider;
  });

  for (let i = 0; i < 10; i++) {
    test(`Positive cases: ${i}`, async () => {
      const result = await _getGasPriorityFee(
        mockProvider,
        blocks,
        DEFAULT_PERCENTAGE
      );
...
describe("[Mock api call - negative cases]: _getGasPriorityFee function", () => {
  test(`should be throw GetGasPriorityFeeError from 'send' method`, async () => {
    const mockProvider = {
      send: (...args: any[]): Promise<FeeHistory> =>
        Promise.reject(new GetGasPriorityFeeError()),
      getBlock: (...args: any[]): Promise<Block> =>
        Promise.resolve({ baseFeePerGas: parseUnits("1", "gwei") } as Block),
    } as JsonRpcProvider;

    const call = async () =>
      await _getGasPriorityFee(
        mockProvider,
        DEFAULT_BLOCKS,
        DEFAULT_PERCENTAGE
      );
...
function generateRewards(blocks: number): {
  ...
  const rewards = Array(blocks)
    ...
    .map(() => {
      return new Array(3)
        .fill(null)
        .map(() => toBeHex(parseUnits(generateRandomNumber(), "gwei"))) as [
        string,
        string,
        string
      ];
